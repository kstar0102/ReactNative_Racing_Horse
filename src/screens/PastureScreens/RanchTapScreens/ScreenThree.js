...
const ScreenThree = ({ threeData, arrowState, showGrowstate }) => {
  ...
  const [banner, setBanner] = useState(threeData[0]);

  useEffect(() => {
    setBanner(threeData[0]);
    setPattern(tiredNumber);
    if (threeData[0].ground == "ダ") {
      setGroundColor("#707172");
    } else if (threeData[0].ground == "芝") {
      setGroundColor("#1BFF00");
    } else if (threeData[0].ground == "万") {
      setGroundColor("red");
    }
  }, [threeData]);
